...
const AppHeader = (props) => {
  const appTitle = useSelector((store) => store.movie_reducer.appTitle)
  const displayFavorites = useSelector((store) => store.favorite_reducer.durum);
    ...
    <div className="flex justify-between items-center shadow rounded-md bg-white p-2 pl-3 my-3">
      ...
      <div className="flex items-center gap-2">
        <div className="myButton bg-blue-600 hover:bg-blue-500">
          <span onClick={() => toggleFavorite()}>Favorileri {displayFavorites ? "gizle" : "göster"}</span>
        </div>
